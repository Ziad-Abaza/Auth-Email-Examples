...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($new_password !== $confirm_password) {
        $error = 'تأكيد كلمة المرور لا يتطابق مع كلمة المرور الجديدة. يُرجى المحاولة مرة أخرى.';
        exit;
    }
    ...
    if (mysqli_num_rows($result_check_reset_token) === 1) {
        ...
        if (mysqli_query($conn, $sql_reset_password)) {
            ...
            $error = 'حدث خطأ أثناء تغيير كلمة المرور. يُرجى المحاولة مرة أخرى.';
        }
    } else {
        $error = 'رمز إعادة تعيين كلمة المرور غير صالح أو انتهت صلاحيته.';
    }
}
?>
