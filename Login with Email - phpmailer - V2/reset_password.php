...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_num_rows($result_check_email) === 1) {
        ...
        if (mysqli_query($conn, $sql_save_reset_token)) {
            ...
            $message = 'لإعادة تعيين كلمة المرور، انقر على الرابط التالي: http://127.0.0.1/project/login/3/reset_password_form.php?email=' . urlencode($email) . '&token=' . urlencode($reset_token);
